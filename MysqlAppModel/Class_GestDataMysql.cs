	...
	class GestClient
	{
		...
		public bool Lire()
		{
			try
			{
				...
				while (Curseur.Read())
				{
					...
					if (Curseur["datecontact"] is System.DBNull)	{ this.datecontact = DateTime.MinValue; } else	{ this.datecontact = Convert.ToDateTime(Curseur["datecontact"]); };
		...
		public bool Ajouter()
		{
			...
			//	string Query = "insert into client(codeclient,nomclient) values( @Param_CodeClient, @Param_NomClient  );";
				...
				MySqlParameter Param_Datecontact = new MySqlParameter("@Param_Datecontact", MySqlDbType.Date);
				...
				Param_Datecontact.Value = this.datecontact.ToString("yyyy-MM-dd");
				...
				Param_Datecontact.Direction = ParameterDirection.Input;
				...
				MyCommand.Parameters.Add(Param_Datecontact);
